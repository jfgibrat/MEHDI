...
void sanity_check(int NberLongReads, int LongestReadLength, int Kmer_len, uint32_t HashMaxSize)
{
  ...
  if(dd > 255) {
    nerr++;
    fprintf(stderr,"The maximum value that member 'kmer_tag' in structure node can take is %llu. It will overflow\n",dd);
  ...
  if(LongestReadLength - Kmer_len > 65535) { // Note: the position in the reads starts at 0
    nerr++;
    fprintf(stderr,"The maximum value that member 'position' in structure node can take is %d. It will overflow\n",LongestReadLength);
  ...
  if(NberLongReads > 65535) {
    nerr++;
    fprintf(stderr,"The maximum value that member 'LongReadNumber' in structure node can take is %d. It will overflow\n",NberLongReads);
    fprintf(stderr,"Upgrade the type of this member from uint16_t to uint32_t in file hashtable.c and recompile the program\n");
  }
